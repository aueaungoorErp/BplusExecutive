...
const ShowBank = ({
  ...
  useEffect(() => {
    ...
    for (var i in arrayObj) {
      newsum += Number(arrayObj[i].forward);
    }
    setSum(newsum);
  }, [arrayObj]);
  const InCome = async () => {
  ...
  return <>
        ...
        flex: 1
                    ...
                    alignSelf: 'center'
                  }}>{Language.t('report.accountName')}</Text></View>
                                    <View width={deviceWidth * 0.4} style={tableStyles.tableHeaderTitle}><Text style={{
                    fontSize: FontSize.medium,
                    color: Colors.fontColor2,
                    alignSelf: 'center'
                  }}>{Language.t('report.accountBalance')}</Text></View>
                                    <View width={deviceWidth * 0.4} style={tableStyles.tableHeaderTitle}><Text style={{
                    fontSize: FontSize.medium,
                    color: Colors.fontColor2,
                    alignSelf: 'center'
                                    ...
                                    <ScrollView>
                                        <KeyboardAvoidingView keyboardVerticalOffset={1}>
                                            <TouchableNativeFeedback>
                                                <View>
                                                    {arrayObj.map(item => {
                          return <View style={tableStyles.tableCell}>
