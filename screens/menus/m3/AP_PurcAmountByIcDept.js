...
const AP_PurcAmountByIcDept = ({
  ...
  useEffect(() => {
    ...
    for (var i in arrayObj) {
      newsum += Number(arrayObj[i].purcAmount);
    }
    setSum(newsum);
  }, [arrayObj]);
  ...
  const fetchInCome = async tempGuid => {
    ...
    await fetch(databaseReducer.Data.urlser + '/Executive', {
      ...
      body: JSON.stringify({
        ...
        'BPAPUS-FUNCTION': 'SHOWICDEPTPURCHASEBYAPKEY',
        'BPAPUS-PARAM': '{"FROM_DATE": "' + sDate + '","TO_DATE": "' + eDate + '","AP_KEY": ' + route.params.Obj + '}',
        'BPAPUS-FILTER': '',
        'BPAPUS-ORDERBY': '',
        'BPAPUS-OFFSET': '0',
        'BPAPUS-FETCH': '0'
      ...
      if (responseData.RECORD_COUNT > 0) {
        for (var i in responseData.SHOWICDEPTPURCHASEBYAPKEY) {
          let jsonObj = {
            id: i,
            code: responseData.SHOWICDEPTPURCHASEBYAPKEY[i].ICDEPT_CODE,
            icdept_thaidesc: responseData.SHOWICDEPTPURCHASEBYAPKEY[i].ICDEPT_THAIDESC,
            purcAmount: responseData.SHOWICDEPTPURCHASEBYAPKEY[i].SHOWPURCAMOUNT
          };
...
export default AP_PurcAmountByIcDept;