...
const Arcat = ({ route }) => {
    ...
    useEffect(() => {
        ...
        for (var i in arrayObj) {
            newsum += Number(arrayObj[i].sumnet)
        }
    ...
    const fetchInCome = async (tempGuid) => {
        ...
        await fetch(databaseReducer.Data.urlser + '/Executive', {
            ...
            body: JSON.stringify({
                ...
                'BPAPUS-FUNCTION': 'SHOWARCATBALANCE',
                'BPAPUS-PARAM':
                    '{ "TO_DATE": "' +
                    eDate + '"}',
                'BPAPUS-FILTER': '',
            ...
            .then((json) => {
                ...
                if (responseData.RECORD_COUNT > 0) {
                    for (var i in responseData.SHOWARCATBALANCE) {
                        let jsonObj = {
                            id: i,
                            code: responseData.SHOWARCATBALANCE[i].ARCAT_CODE,
                            name: responseData.SHOWARCATBALANCE[i].ARCAT_NAME,
                            sumamount: responseData.SHOWARCATBALANCE[i].SUMAMOUNT,
                            sumallpayment: responseData.SHOWARCATBALANCE[i].SUMALLPAYMENT,
                            sumnet: responseData.SHOWARCATBALANCE[i].SUMAMOUNT - responseData.SHOWARCATBALANCE[i].SUMALLPAYMENT,
                        };
                                                        ...
                                                        <>
                                                            <View style={tableStyles.tableCell}>
                                                                ...
                                                                <View width={deviceWidth * 0.4} style={tableStyles.tableCellTitle}><Text style={{
                                                                    ...
                                                                    color: Colors.fontColor,
                                                                    alignSelf: 'flex-end'
                                                                }} >{safe_Format.currencyFormat(item.sumallpayment)}</Text></View>
                                                                <View width={deviceWidth * 0.4} style={tableStyles.tableCellTitle}><Text style={{
                                                                    fontSize: FontSize.medium,
                                                                    color: Colors.fontColor,
                                                                    alignSelf: 'flex-end'
                                                                }} >{safe_Format.currencyFormat(item.sumnet)}</Text></View>
                                                            </View>
                                                        </>
                                                    )
                                                })}
