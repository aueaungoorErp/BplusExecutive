...
const OrderInformation = ({ route }) => {
  ...
  const [btn, setBtn] = useState([
    { name: 'ประวัติการซื้อ', state: true },
    { name: 'ประวัติการขาย', state: false },
    { name: 'จำนวนคงเหลือ', state: false },
    { name: 'ราคาขาย', state: false },
  ]);
  ...
  const regisMacAdd = async () => {
    let tempGuid = await safe_Format._fetchGuidLog(
  ...
  const getSKUINFOBYGOODSCODE = async tempGuid => {
    await fetch(databaseReducer.Data.urlser + '/SetupErp', {
      ...
      body: JSON.stringify({
        ...
        'BPAPUS-PARAM':
          '{"GOODS_CODE": "' + route.params.data.GOODS_CODE + '"}',
        'BPAPUS-FILTER': '',
        'BPAPUS-ORDERBY': '',
        'BPAPUS-OFFSET': '0',
        'BPAPUS-FETCH': '0',
  ...
  const fetchSHOWWLSKUQTYBYGOODSKEY = async tempGuid => {
    ...
    await fetch(databaseReducer.Data.urlser + '/Executive', {
      ...
      body: JSON.stringify({
        ...
        'BPAPUS-PARAM':
          '{"TO_DATE": "' +
          safe_Format.setnewdateF(safe_Format.checkDate(end_date)) +
          '","GOODS_KEY": "' +
          route.params.data.GOODS_KEY +
          '"}',
        'BPAPUS-FILTER': '',
        'BPAPUS-ORDERBY': '',
        'BPAPUS-OFFSET': '0',
      ...
      .then(json => {
        ...
        if (responseData.RECORD_COUNT > 0) {
          console.log(
            'SHOWWLSKUQTYBYGOODSKEY >> ',
            responseData.SHOWWLSKUQTYBYGOODSKEY,
          );
          const unique = [
            ...new Set(
              responseData.SHOWWLSKUQTYBYGOODSKEY.map(item =>
                JSON.stringify(item),
              ),
            ),
      ...
      .then(async json => {
        ...
        if (responseData.RECORD_COUNT > 0) {
          ...
          for (var i in responseData.Sp000211) {
            await fetchSp000221(
              tempGuid ? tempGuid : loginReducer.guid,
              responseData.Sp000211[i],
            );
          }
        }
      })
  ...
  const fetchSp000221 = async (tempGuid, Sp211) => {
          ...
          if (responseData.RECORD_COUNT > 0) {
            ...
            if (Sp000221.length > 0) {
              let obj = {
                WPurcPrice: Sp211,
                Sp000221: Sp000221,
                SKU_STD_COST: await READSKU(
                  'SKU_STD_COST',
                  GoodsInfo.DOCINFO.SKU_KEY,
                ),
                SKU_STD_COST_TY:
                  Number(
                    await READSKU('SKU_STD_COST', GoodsInfo.DOCINFO.SKU_KEY),
                  ) +
                  Number(
                    await READSKU('SKU_COST_TY', GoodsInfo.DOCINFO.SKU_KEY),
                  ),
              };
    ...
    const unique = [...new Set(Sp000211.map(item => JSON.stringify(item)))].map(
  ...
  const fetchOe404 = async tempGuid => {
    ...
    await fetch(databaseReducer.Data.urlser + '/LookupErp', {
      ...
      body: JSON.stringify({
        ...
        'BPAPUS-FILTER':
          "AND ( DI_DATE  >='" +
          safe_Format.setnewdateF(safe_Format.checkDate(start_date)) +
          "') AND ( DI_DATE  <='" +
          safe_Format.setnewdateF(safe_Format.checkDate(end_date)) +
          "') ",
      ...
      .then(async json => {
        ...
        if (responseData.RECORD_COUNT > 0) {
          ...
          for (var i in responseData.Oe000404) {
            await GetReceiveDocinfo(
              tempGuid ? tempGuid : loginReducer.guid,
              responseData.Oe000404[i],
            );
          }
        }
      })
  ...
  const GetReceiveDocinfo = async (tempGuid, Oe000404) => {
    ...
    const unique = [...new Set(Oe404Obj.map(item => JSON.stringify(item)))].map(
      item => JSON.parse(item),
    );
      ...
      .then(async json => {
        ...
        if (responseData.RECORD_COUNT > 0) {
          ...
          for (var i in responseData.Oe000304) {
            await GetInvoiceDocinfo(
              tempGuid ? tempGuid : loginReducer.guid,
              responseData.Oe000304[i],
            );
          }
        }
      })
  ...
  const GetInvoiceDocinfo = async (tempGuid, Oe000304) => {
    ...
    const unique = [...new Set(Oe304Obj.map(item => JSON.stringify(item)))].map(
      item => JSON.parse(item),
    );
  ...
  const sellC = (a, s) => {
    console.log(`a ${a} b ${s}`);
    if (s.indexOf('b') > -1 || s.indexOf('B') > -1 || s.indexOf('บ') > -1)
      return sellB(s);
                          ...
                          >
                            ...
                            <CalendarScreen
                              ...
                              calendarModel={{
                                ...
                                pickItem: { color: Colors.itemColor },
                              }}
                              borderWidth={1}
                          ...
                          <>
                            ...
                            <ScrollView>
                              {arrayOe404Obj
                                .sort((a, b) => {
                                  return (
                                    b.Oe000404.DI_DATE - a.Oe000404.DI_DATE
                                  );
                                })
                                .map(item => {
                                    ...
                                    return (
                                      <View style={{ flexDirection: 'row' }}>
                                        ...
                                        <View width={deviceWidth * 0.3}>
                                          ...
                                          >
                                            {safe_Format.currencyFormat(
                                              Number(Transtkditem.TRD_N_SELL) /
                                                (Number(
                                                  Transtkditem.TRD_UTQQTY,
                                                ) *
                                                  Number(Transtkditem.TRD_QTY)),
                                            )}
                                          </Text>
                                        </View>
                                        <View width={deviceWidth * 0.5}>
                          ...
                          <>
                            ...
                            <ScrollView>
                              {arrayOe304Obj
                                .sort((a, b) => {
                                  return (
                                    b.Oe000304.DI_DATE - a.Oe000304.DI_DATE
                                  );
                                })
                                .map(item => {
                                  return item.Transtkd.map(Transtkditem => {
                          ...
                          <>
                            ...
                            <ScrollView>
                              {arraySp000211
                                .sort((a, b) => {
                                  return (
                                    a.WPurcPrice.ARPRB_CODE -
                                    b.WPurcPrice.ARPRB_CODE
                                  );
                                })
                                .map(item => {
                                  return (
                                      ...
                                      return (
                                        <View style={{ flexDirection: 'row' }}>
                                          ...
                                          <View width={deviceWidth * 0.3}>
                                            ...
                                            >
                                              {Sp000221items.ARPLU_U_PRC == 0
                                                ? '-'
                                                : safe_Format.currencyFormat(
                                                    Sp000221items.ARPLU_U_PRC,
                                                  )}
                                            </Text>
                                          </View>
                                          <View width={deviceWidth * 0.3}>
