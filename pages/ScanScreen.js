...
import { ActivityIndicator, Alert, Image, ImageBackground, Modal, PermissionsAndroid, Platform, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
const ScanScreen = ({
  ...
  const extractQrFields = payload => {
    ...
    try {
      ...
      if (jsonData && typeof jsonData === 'object') {
        return Object.entries(jsonData).reduce((result, [key, value]) => {
    ...
    const extractedPairs = normalizedPayload.split(/[|\n;,]+/).map(part => part.trim()).filter(Boolean).reduce((result, part) => {
      const separatorIndex = part.indexOf('=') > -1 ? part.indexOf('=') : part.indexOf(':');
      if (separatorIndex === -1) {
        return result;
      }
  ...
  const withTimeout = (promise, timeoutMs) => Promise.race([promise, new Promise((_, reject) => setTimeout(() => reject(new Error('QR_TIMEOUT')), timeoutMs))]);
  ...
  const parseQrPayload = data => {
    if (!data) {
      openQrError(Language.t('alert.errorTitle'), `${Language.t('selectBase.notfound')}\n\nEmpty QR payload`);
      return;
    }
    const decodedCandidates = decodePayloadCandidates(data);
    const matchedPayload = decodedCandidates.find(item => typeof item === 'string' && item.indexOf('.dll') !== -1);
    if (!matchedPayload) {
      openQrError(Language.t('alert.errorTitle'), `${Language.t('selectBase.invalid')}\n\nRaw: ${String(data)}`);
      return;
    }
  ...
  const decodeImage = async asset => {
    ...
    try {
      ...
      const attemptDetails = Array.isArray(error?.attemptDetails) ? error.attemptDetails.join('\n') : '';
      openQrError(Language.t('alert.errorTitle'), [Language.t('selectBase.notfound'), `Reason: ${getErrorMessage(error)}`, attemptDetails ? `Attempts:\n${attemptDetails}` : ''].filter(Boolean).join('\n\n'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return <ImageBackground source={backgroundImage} resizeMode="cover" style={styles.page}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
          <Image style={styles.backIcon} resizeMode="contain" source={require('../img/iconsMenu/arrow.png')} />
        </TouchableOpacity>
      ...
      <View style={styles.content}>
        <View style={styles.card}>
          <Image style={styles.heroIcon} resizeMode="contain" source={require('../img/iconsMenu/qr-code.png')} />
      ...
      <Modal transparent visible={Boolean(qrError.detail)} animationType="fade" onRequestClose={closeQrError}>
        <View style={styles.loadingOverlay}>
