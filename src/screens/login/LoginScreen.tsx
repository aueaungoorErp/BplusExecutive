...
import { StyleSheet, BackHandler, View, Text, Button, TouchableOpacity, TextInput, Image, KeyboardAvoidingView, ScrollView, LogBox, ToastAndroid, StatusBar, Platform } from 'react-native';
...
class LoginScreen extends Component {
        ...
        marginTop: 20
          ...
          marginTop: 5
            ...
            height: 30
              ...
              <Image source={require('../../images/ico_about.png')} resizeMode="stretch" style={{
              width: 30,
              height: 30,
              flex: 1
            }} />
            ...
            height: 30
              ...
              <Image source={require('../../images/ico_setting.png')} resizeMode="stretch" style={{
              width: 30,
              height: 30,
              flex: 1
            }} />
  ...
  _FetchApiData = async () => {
    try {
      ...
      const valueINPUT_RADIO_SET = await AsyncStorage.getItem('INPUT_RADIO_SET');
      if (valueINPUT_RADIO_SET === 'A' && valueINPUT_RADIO_SET !== null) {
        this.props.getfetchAuthApi(valueINPUT_1, this.state.user, this.state.pass);
      } else if (valueINPUT_RADIO_SET === 'B' && valueINPUT_RADIO_SET !== null) {
        this.props.getfetchAuthApi(valueINPUT_2, this.state.user, this.state.pass);
      } else if (valueINPUT_RADIO_SET === 'C' && valueINPUT_RADIO_SET !== null) {
        this.props.getfetchAuthApi(valueINPUT_3, this.state.user, this.state.pass);
      } else if (valueINPUT_RADIO_SET === 'D' && valueINPUT_RADIO_SET !== null) {
        this.props.getfetchAuthApi(valueINPUT_4, this.state.user, this.state.pass);
      }
    } catch (error) {
      console.error(error);
    }
  };
  renderModalContent = () => <View style={{
    flex: 1,
  ...
  render() {
    if (this.props.fetchReducer.dataApi) {
      if (this.props.fetchReducer.dataApi.ReadUserKey) {
        if (this.props.fetchReducer.dataApi.ReadUserKey.length > 0) {} else {}
      } else {}
    } else {}
          ...
          height: '100%'
            ...
            height: '100%'
              ...
              alignItems: 'center'
                ...
                <Image source={require('../../images/pic_logo_app.png')} resizeMode="stretch" style={{
                width: 200,
                height: 200,
                alignItems: 'center'
              }} />
              ...
              <View style={styles.inputWrapper}>
                <FontAwesomeIcon name="user-circle" size={18} color="#f95a25" style={styles.inputIcon} />
                <Fumi ref={ref => {
                this.inputRefs['1'] = ref;
              }} style={styles.input} label={'UserName'} labelStyle={[styles.inputLabel, {
                color: '#a3a3a3'
              }]} inputStyle={styles.inputText} blurOnSubmit={false} returnKeyType="next" onChangeText={this.onChangeUser_.bind(this)} value={this.state.user} onSubmitEditing={() => this._focusNextField('2')} />
              </View>
              <View style={styles.inputWrapper}>
                <FontAwesomeIcon name="bullseye" size={18} color="#f95a25" style={styles.inputIcon} />
                <Fumi ref={ref => {
                this.inputRefs['2'] = ref;
              }} style={styles.input} label={'PassWord'} labelStyle={styles.inputLabel} inputStyle={styles.inputText} returnKeyType="done" secureTextEntry={this.state.secureTextEntry} onChangeText={this.onChangePass_.bind(this)} value={this.state.pass} onSubmitEditing={() => this._FetchApiData()} />
                <TouchableOpacity style={styles.eyeIconButton} onPress={this.updateSecureTextEntry}>
                  <MaterialCommunityIcons name={this.state.secureTextEntry ? 'eye-off' : 'eye'} size={22} color="#f95a25" />
                </TouchableOpacity>
              </View>
            ...
            {this.props.fetchReducer.dataApi ? this.props.fetchReducer.dataApi.ReadUserKey ? this.props.fetchReducer.dataApi.ReadUserKey.length > 0 ? this.goToScreen() : <Text style={{
            alignItems: 'center',
            alignSelf: 'center',
            color: '#fe8a01'
          }}>
            ...
            color: '#fe8a01'
                ...
                </Text> : <Text>dataApi == null</Text>}

            <ProgressDialog visible={this.props.fetchReducer.isApiData_Fetching} title="Loading" message="Please, wait..." />
          </View>
